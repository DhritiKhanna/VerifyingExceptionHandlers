...
public class WebSocketHandshake {
	...
	private void sendHandshakeRequest(String key) throws IOException{
		try {
			...
			if (srvUri.getRawPath() != null && /*!srvUri.getRawPath().isEmpty()*/ srvUri.getRawPath().length()!=0) { // DK: I don't know why isEmpty() is not working
				path = srvUri.getRawPath();
				if (srvUri.getRawQuery() != null && /*!srvUri.getRawQuery().isEmpty()*/ srvUri.getRawQuery().length()!=0) { // DK: I don't know why isEmpty() is not working
