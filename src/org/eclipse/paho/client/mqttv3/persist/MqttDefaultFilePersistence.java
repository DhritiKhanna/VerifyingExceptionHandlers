...
public class MqttDefaultFilePersistence implements MqttClientPersistence {
	...
	public void open(String clientId, String theConnection) throws MqttPersistenceException {
		...
		synchronized (this) {
			...
			try {
				//fileLock = new FileLock(clientDir, LOCK_FILENAME); // DK
	 		} catch (Exception e) {
	...
	private File[] getFiles() throws MqttPersistenceException {
		...
		//System.out.println("~~~~~~~~~~~~~~~ getFiles() in MqttDefaultFilePersistence: " + clientDir.exists());
		//System.out.println("clientDir name: " + clientDir.getName());
		//if(files != null) System.out.println("files size: " + files.length);
		if (files == null) {
			throw new MqttPersistenceException();
		}
		return files;
