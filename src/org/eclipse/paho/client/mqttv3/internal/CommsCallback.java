...
//import org.eclipse.paho.client.mqttv3.logging.Logger; // DK
//import org.eclipse.paho.client.mqttv3.logging.LoggerFactory; //DK
// DK --> Removed all logging from the file
	...
	public void run() {
		...
		while (running) {
			try {
				...
				running = false;
				System.out.println("$$$$$$$$$$$ About to call clientComms.shutdownConnection from CommsCallback");
	...
	public void asyncOperationComplete(MqttToken token) {
		...
		if (running) {
			...
			try {
				...
				ex.printStackTrace();
				System.out.println("$$$$$$$$$$$ About to call clientComms.shutdownConnection");
				clientComms.shutdownConnection(null, new MqttException(ex));
			}
