...
public class CommsSender implements Runnable {
			...
			try {
				...
				if (message != null) {
					...
					if (message instanceof MqttAck) {
						System.out.println("Message written: " + message + " from thread: " + Thread.currentThread().getName());
						out.write(message);
						out.flush();
					} else {
	...
	private void handleRunException(MqttWireMessage message, Exception ex) {
		...
		running = false;
		System.out.println("$$$$$$$$$$$ About to call clientComms.shutdownConnection from CommsSender");
